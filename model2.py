...
def get_confussion_matrix(classified):
    ...
    confusion_matrix = pd.crosstab(matrix_data['y_actual'], matrix_data['y_predicted'], rownames=['Actual'],
                                   colnames=['Predicted'])
...
for cycle in range(5):
    ...
    for row in test_data:
        ...
        med_prob = count_probability(row, 2)
        prob_list.update({2: med_prob})

        # high sick angle

        high_prob = count_probability(row, 3)
        prob_list.update({3: high_prob})

        # sick angle

        sick_prob = count_probability(row, 4)
        prob_list.update({4: sick_prob})
